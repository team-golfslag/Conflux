...
#pragma warning restore ASP0000
    ...
    private static async Task ConfigureSRAMServices(WebApplicationBuilder builder, IVariantFeatureManager featureManager)
    {
        ...
        builder.Services.AddSingleton<ISCIMApiClient, SCIMApiClient>(provider => {
            ...
            SCIMApiClient scimClient = new SCIMApiClient(client);
    ...
    private static async Task ConfigureAuthentication(WebApplicationBuilder builder, IVariantFeatureManager featureManager)
    {
        bool sramEnabled = await featureManager.IsEnabledAsync("SRAMAuthentication");
        if (sramEnabled)
            SetupAuth(builder);
    ...
    private static void ConfigureCookieAuth(CookieAuthenticationOptions options)
    {
        options.Events.OnSignedIn = context => {
            IUserSessionService userSessionService = context.HttpContext.RequestServices.GetRequiredService<IUserSessionService>();
