...
public static class ProductMapper
{
    ...
    public static List<Product> MultipleRepoItemsToProducts(List<SURFSharekitRepoItem> repoItems)
    {
        ...
        foreach (SURFSharekitRepoItem repoItem in repoItems)
        {
            Product? newProduct = MapProduct(repoItem);
    ...
    {
        if (repoItem.Attributes == null)
        {
            return null;
        }
  
        string? title = repoItem.Attributes.Title;
        if (string.IsNullOrEmpty(title)) //product requires a title to be made
        {
            return null;
        } 
